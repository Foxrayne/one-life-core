...
| `Storm.AnimalZoneSafehouseProtection` | `AnimalZoneSafehouseGuard#setEnabled` | **Boolean.** Server authority over animal-zone edits: a zone overlapping a safehouse is editable only by that safehouse's owner/members (or `CanGoInsideSafehouses` roles); group removals drop only the editable zones and the server re-sends the protected ones. Default `false` = vanilla (any player edits any zone). Live-appliable. Auto-reverts permanently if the guard throws. Gauge `storm_animal_zone_safehouse_protection` (1/0). |
